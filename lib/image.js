...
var ak = configs.apiKey;//'gyFG80njksqzyzwyiDxc3Irv';//'yourak';
var sk = configs.secretKey;//'do6Blf9QzzYpTBsjKX5K1RNqb5odcefG';//'yoursk';
...
function testVCode(req, res) {
  ...
  imageService.generateVCode(option, function(err, result) {
    ...
    req.session.vcode_imgurl = result['response_params'].imgurl;
    req.session.vcode_str = result.response_params.vcode_str;
    //res.cookie('vcode_str', result['response_params'].vcode_str,{httpOnly: true,maxAge: 1000 * 60 * 60 * 24 * 30,signed: true });
